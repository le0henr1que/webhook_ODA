...
class WhatsAppSender {
  ...
  _setupEvents() {
    ...
    self.eventsEmitter.on(
      Config.EVENT_QUEUE_MESSAGE_TO_WHATSAPP,
    ...
    self.eventsEmitter.on(
      Config.EVENT_WHATSAPP_MESSAGE_DELIVERED,
      function (messageId) {
    ...
    self.eventsEmitter.on(
      Config.EVENT_PROCESS_NEXT_WHATSAPP_MESSAGE,
      function () {
  ...
  async _sendMessageToWhatsApp(message) {
    let self = this;
    try {
      console.log("Message to Whatsapp: " + JSON.stringify(message));
      const config = {
        method: "post",
  ...
  async _downloadAndSaveWhatsAppAttachmentMessage(attachment) {
    try {
      ...
      const config = {
        method: "get",
        url:
          `${self.whatsAppApiUrl}/${self.whatsAppApiVersion}/` + attachment.id,
        headers: {
          Authorization: `Bearer ${self.whatsAppAccessToken}`,
          "Content-Type": "application/json",
        },
      ...
      const fileName = "file_"
        .concat(Date.now())
        .concat(".")
        .concat(mime.extension(attachment.mime_type));
      await attachmentResponse.data.pipe(
        fs.createWriteStream(
          path.join(__dirname, "../../downloads/").concat(fileName)
        )
      );
      return fileName;
    } catch (error) {
